...
int find_compaction_index(distribution_t *dist, int *idx) {
    RT_ASSERT(dist->rt, is_full(dist), "Error: Distribution is not full");
    if (dist->rt->has_error) {
        return EXIT_FAILURE;
    }
    ...
    *idx = INT_MAX;
    ...
    for (int i = 0; i < candidate_count; i++) {
        int compaction_idx = (dist->last_insert_idx + i * dist->curr_gap) % (dist->bin_count - 2);
...
int compact(distribution_t *dist) {
    ...
    dist->bins[compaction_idx] = interpolate(dist, compaction_idx);
